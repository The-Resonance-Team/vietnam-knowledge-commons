...
module.exports = {
  forbidden: [
    ...
    // --- Layering -------------------------------------------------------------
    // cli may depend on sdk and schemas; sdk may depend on schemas;
    // schemas depends on nothing. No upward edges.
    {
      name: "layering",
      comment: "Dependency direction is schemas ← sdk ← cli. No reverse edges.",
      severity: "error",
      from: { path: `^${R}/schemas/` },
      to: { path: `^${R}/(sdk|cli)/` },
    },
    {
      name: "layering-sdk-no-cli",
      comment: "sdk must not depend on cli.",
      severity: "error",
      from: { path: `^${R}/sdk/` },
      to: { path: `^${R}/cli/` },
    },
